...
<script>
    $(document).ready(function() {
        $('#upOtmEvaluationForm').bootstrapValidator({
...
<div class="col-xs-6">
    <div class="form-group">
        ...
        <g:select id="schemeInfo" name="schemeInfo.id" from="${SchemeInfo.findAllWhere(IMPLEMENTATION_SYSTEM :'OTM Procurement')}" optionKey="id" optionValue="NAME" noSelection="${['':'Select One...']}" required="" value="${upOtmEvaluationInstance?.schemeInfo?.id}" onchange="setValueOnProcurementChange(this.value)" class="form-control"/>
    </div>
</div>

<div class="col-xs-6">
    <div class="form-group">
        <label for="TEC"><g:message code="upOtmEvaluation.TEC.label" default="দরপত্র মূল্যায়ন কমিটি" /></label>
        <g:select id="TEC" name="TEC.id" from="${settings.Committee.list()}" optionKey="id" optionValue="COMMITTEE_NAME" required="" value="${upOtmEvaluationInstance?.TEC?.id}" class="form-control"/>
    </div>
</div>
...
<div class="col-xs-12 detail-list ${hasErrors(bean: upOtmEvaluationInstance, field: 'upOtmEvaluationSheetDetails', 'error')} ">
    <div class="form-group">
